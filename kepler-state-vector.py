...
if __name__ == "__main__":
    ...
    print("X = {} AU".format(sp.convrt(state[0], 'km', 'AU')))
    print("Y = {} AU".format(sp.convrt(state[1], 'km', 'AU')))
    print("Z = {} AU".format(sp.convrt(state[2], 'km', 'AU')))
    print("")
    print("# Velocity of Kepler in the geocentric J2000 frame")
    print("dX = {} AU/s".format(sp.convrt(state[3], 'km', 'AU')))
    print("dY = {} AU/s".format(sp.convrt(state[4], 'km', 'AU')))
    print("dZ = {} AU/s".format(sp.convrt(state[5], 'km', 'AU')))
    # As a sanity check, print the speed
    speed = (state[3]**2 + state[4]**2 + state[5]**2)**.5
    print("")
    print("# Orbital speed [sqrt(dX^2 + dY^2 + dZ^2)]")
    print("Speed = {} AU/s".format(sp.convrt(speed, 'km', 'AU')))
